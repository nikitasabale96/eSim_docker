...
/**
 * @file
 * Contains \Drupal\hackathon_submission\Form\EditSocMarathonLiteratureReportSubmissionForm.
 */
...
class EditSocMarathonLiteratureReportSubmissionForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'edit_soc_marathon_literature_report_submission_form';
  }
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state, $no_js_use = NULL) {
    ...
    //                 'file_validate_extensions' => array(variable_get('soc_marathon_literature_report_extensions', '')),
    //                 // Pass the maximum file size in bytes
    //                 /*'file_validate_size' => array(5*1024*1024),
    //               ),*/
    //         '#description' => t('No spaces or any special characters allowed in filename.') . '<br />' . t('<span style="color:red;">Current File:</span> ') . $report_file . '<br />' . t('<span style="color:red;">Allowed file extensions: ') . variable_get('soc_marathon_literature_report_extensions', '') . '</span>'
    //     );
        ...
        if ($file_name) {
          ...
          switch ($file_type) {
            case 'L':
              ...
              // // This looks like another module's variable. You'll need to rewrite this call
              // // to ensure that it uses the correct configuration object.
              // $allowed_extensions_str = variable_get('soc_marathon_literature_report_extensions', '');
          ...
          if (!in_array($temp_extension, $allowed_extensions)) {
            $form_state->setErrorByName($file_form_name, t('File with ' . $allowed_extensions_str . ' extension only can be uploaded.'));
          }
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    $query = "UPDATE mixed_signal_soc_marathon_literature_survey SET 
          ...
          case 'L':
            if (file_exists($root_path . $dest_path . $_FILES['files']['name'][$file_form_name])) {
              ...
              if (move_uploaded_file($_FILES['files']['tmp_name'][$file_form_name], $root_path . $dest_path . $_FILES['files']['name'][$file_form_name])) {
                $query_ab_f = "SELECT * FROM mixed_signal_soc_marathon_literature_survey WHERE id = :submission_id";
